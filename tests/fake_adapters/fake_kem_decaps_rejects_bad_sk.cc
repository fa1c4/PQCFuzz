...
pqcfuzz_status Decaps(uint8_t *ss, const uint8_t *, const uint8_t *sk) {
  if (std::memcmp(sk, g_sk, 4) != 0) {
...
extern "C" const pqcfuzz_kem_adapter *pqcfuzz_fake_kem_decaps_rejects_bad_sk_adapter() {
  static const pqcfuzz_kem_adapter adapter = {
      "fake", "fake_kem_decaps_rejects_bad_sk", "ML-KEM-768", 4, 4, 4, 4, Keygen, Encaps, Decaps};
  return &adapter;
}
