#!/usr/bin/env bash
# PQCFuzz evaluation driver for the pinned NTRU round-3 reference.
#
# Commands:
#   scripts/pqcfuzz_ntru_eval.sh build      # generate jobs + build fuzzers/replays
#   scripts/pqcfuzz_ntru_eval.sh preflight  # seed corpus + -runs=1 + coverage gate
#   scripts/pqcfuzz_ntru_eval.sh smoke      # deterministic corpus + short sanitizer fuzz
#   scripts/pqcfuzz_ntru_eval.sh run        # budgeted campaign
#   scripts/pqcfuzz_ntru_eval.sh report     # write workspace/ntru/report/summary.{json,md}
#
# Environment:
#   PAIR_ALG=src/config/pair_alg.ntru.json
#   WORK_ROOT=workspace/ntru
...
PAIR_ALG="${PAIR_ALG:-src/config/pair_alg.ntru.json}"
WORK_ROOT="${WORK_ROOT:-workspace/ntru}"
...
NTRU_SRC="${NTRU_SRC:-projects/NTRU/reference}"
NTRU_KAT="${NTRU_KAT:-projects/NTRU/kat}"
SOURCE_LOCK="${SOURCE_LOCK:-src/config/source_locks/ntru.json}"
SCHEME_PROFILE="${SCHEME_PROFILE:-src/config/scheme_profiles/ntru.json}"
...
COMMON_ARCHIVE="${BUILD_DIR}/libpqcfuzz_ntru_common.a"
...
usage() {
  cat <<'EOF'
Usage: scripts/pqcfuzz_ntru_eval.sh {build|preflight|smoke|run|report}
EOF
}
...
job_files() {
  local job_file
  for job_file in "$JOBS_DIR"/job_ntru_*.json; do
    [ -e "$job_file" ] || continue
...
source_dir_for_job() {
  python3 - "$SCHEME_PROFILE" "$1" <<'PY'
import json
import sys

profile_path, algorithm = sys.argv[1], sys.argv[2]
...
for entry in payload["parameter_sets"]:
    if entry["algorithm"] == algorithm:
        ...
        raise SystemExit(0)
raise SystemExit(f"algorithm not in profile: {algorithm}")
PY
}
...
with open(sys.argv[1], encoding="utf-8") as fh:
    job = json.load(fh)
name = job["algorithm"].lower().replace("-", "_")
print(name + "_")
PY
}
...
build_common_archive() {
  ...
  wait || status=$?
  if [ "$status" -ne 0 ]; then
    echo "[ntru] common archive build failed" >&2
    exit 1
  fi
  rm -f "$COMMON_ARCHIVE"
...
build_job() {
  ...
  local config_file pair_id pk_exchange ct_exchange
...
payload = {
    ...
    "build_flags": {
        ...
        "drbg": "NIST AES256-CTR-DRBG (katrng) for the KAT lane",
    },
}
with open(path, "w", encoding="utf-8") as fh:
    ...
    fh.write("\n")
PY
  echo "[ntru] built $job_id"
}

cmd_build() {
  mkdir -p "$BUILD_DIR" "$RUNS_DIR" "$RESULTS_DIR" "$CRASHES_DIR" "$JOBS_DIR" "$REPORT_DIR"
  if [ ! -f "$SOURCE_LOCK" ]; then
    echo "[ntru] missing source lock: $SOURCE_LOCK" >&2
    exit 1
  fi
  if [ ! -f "$SCHEME_PROFILE" ]; then
    echo "[ntru] missing scheme profile: $SCHEME_PROFILE" >&2
    exit 1
  fi
  if [ ! -f "$NTRU_SRC/ntruhps2048509/kem.c" ]; then
    echo "[ntru] missing vendored reference: $NTRU_SRC" >&2
    echo "[ntru] real-library lanes are unavailable; fake-adapter tests remain runnable" >&2
    exit 1
  fi
  python3 src/jobs/generate_jobs.py --pair-alg "$PAIR_ALG" --algorithm-family NTRU \
...
payload = {
    ...
    "vendored_source": "projects/NTRU/reference",
...
with open(path, "w", encoding="utf-8") as fh:
    json.dump(payload, fh, indent=2, sort_keys=True)
    fh.write("\n")
print(f"[ntru] build manifest: {path} ({len(records)} jobs)")
PY
}
...
for index in range(cases):
    ...
    # kem.ciphertext.coefficient (0x1A) at coefficient index, value aux.
    coefficient_index = index * 5 + 3
    aux = (100 + index * 37) % 4096
    mutation = bytes([0x08, 0x1A]) + struct.pack("<II", coefficient_index, aux)
    message = b""
    extra = b""
...
PY
  ...
  if [ -n "$missing" ]; then
    echo "[ntru] coverage gate failed for oracles: $(echo "$missing" | tr '\n' ' ')" >&2
    return 1
  fi
}
...
preflight_job() {
  ...
  echo "[ntru] preflight ok: $job_id"
  return 0
}

replay_job_seeds() {
  local job_file="$1" replay_bin="$2" corpus_dir="$3" result_dir="$4"
  local algorithm pair_id config_file implementation_id pk_exchange ct_exchange
  algorithm=$(job_field "$job_file" 'j["algorithm"]')
  pair_id=$(job_field "$job_file" 'j["pair_id"]')
  config_file=$(job_field "$job_file" 'j["paths"]["generated_config"]')
  implementation_id=$(left_implementation_id "$job_file")
  ...
  while IFS=: read -r oracle_enum oracle_name; do
    ...
    for attempt in 1 2; do
      ...
      if [ "$rc" -ne 0 ] && [ "$rc" -ne 70 ]; then
        echo "[ntru] replay failed for $oracle_name (rc=$rc) in $(job_id_of "$job_file")" >&2
        return 1
      fi
    done
  done < <(oracle_specs_for_job "$job_file")
...
with open(path, "w", encoding="utf-8") as fh:
    json.dump(payload, fh, indent=2, sort_keys=True)
    fh.write("\n")
print(f"[ntru] preflight coverage gate: {'passed' if status == 0 else 'FAILED'}")
PY
  exit "$status"
}
...
run_job() {
  ...
  echo "[ntru] campaign $job_id finished with rc=$rc"
}

cmd_smoke() {
  python3 -m pytest -q tests/ntru_model_test.py tests/ntru_oracles_test.py::test_official_kat_fixture \
    >"$RUNS_DIR/model-lane.log" 2>&1 || {
    echo "[ntru] model/KAT lane failed; see $RUNS_DIR/model-lane.log" >&2
    exit 1
  }
  local job_file status=0
  while IFS= read -r job_file; do
...
payload = {
    ...
    "sanitizers": ["address", "undefined"],
    "model_lane": "tests/ntru_model_test.py",
    "official_kat": True,
...
with open(path, "w", encoding="utf-8") as fh:
    json.dump(payload, fh, indent=2, sort_keys=True)
    fh.write("\n")
print(f"[ntru] smoke: {'passed' if status == 0 else 'FAILED'}")
PY
  exit "$status"
}
...
jobs = []
for job_path in sorted(Path(jobs_dir).glob("job_ntru_*.json")):
    with open(job_path, encoding="utf-8") as fh:
...
for job in jobs:
    ...
    profiles.append({
        "job_id": job_id,
        "algorithm": job["algorithm"],
        "variant": job["algorithm_metadata"].get("variant", ""),
...
payload = {
    ...
    "source": {
        ...
        "archive_sha256": lock["sources"][0]["archive_sha256"],
        "vendored_path": "projects/NTRU/reference",
        "implementation": "ntru_reference",
    ...
    "not_covered": [
        "independent ref/AVX2 differential lane (ntru_cross_exchange) is disabled in the single-implementation lock",
        "ntru_dpke_membership, ntru_key_algebra, ntru_codec_roundtrip and ntru_dpke_failure_output are evaluated by "
        "the Python model lane plus test-only reference hooks, not by the native fuzzer executor",
        "ntru_ct_padding, ntru_implicit_rejection_exact and ntru_prf_key_separation are NOT_APPLICABLE for "
        "NTRU-HPS-4096-821 because it has no ciphertext padding field; membership-fail fallback for that profile is "
        "covered by the model lane",
        "P2 ntru_fault_checks and ntru_timing_resources are opt-in and not run by default",
        "no IND-CCA2, EUF/sUF or quantum-security proof is claimed",
    ],
...
lines = [
    ...
    "",
    "| profile | variant | oracles entered | findings | fuzz | smoke |",
    "| --- | --- | ---: | ---: | --- | --- |",
]
for profile in profiles:
    lines.append(
        f"| {profile['algorithm']} | {profile['variant']} | {profile['oracles_entered']}/{profile['oracles_scheduled']} | "
...
with open(report_dir / "summary.md", "w", encoding="utf-8") as fh:
    fh.write("\n".join(lines) + "\n")
print(f"[ntru] report: {report_dir / 'summary.json'}")
PY
}
...
case "${1:-}" in
  ...
  run)
    ...
    if [ -z "$selected_jobs" ]; then
      echo "[ntru] no jobs built; run build first" >&2
      exit 1
    fi
    while IFS= read -r job_file; do
      run_job "$job_file"
