...
if [ ! -d "${LIBOQS_SRC_DIR}/.git" ]; then
  ...
  if [ -d "${LIBOQS_LOCAL_REPO}/.git" ]; then
    echo "[CLFuzz] cloning liboqs from local mirror: $LIBOQS_LOCAL_REPO"
    ...
    while [ "$attempt" -le "$LIBOQS_CLONE_RETRIES" ]; do
      echo "[CLFuzz] git clone attempt ${attempt}/${LIBOQS_CLONE_RETRIES}"
    ...
    if [ ! -d "${LIBOQS_SRC_DIR}/.git" ]; then
      echo "[CLFuzz] git clone failed after ${LIBOQS_CLONE_RETRIES} attempts" >&2
      exit 1
    fi
  fi
else
