#ifndef PQCFUZZ_MUTATORS_ENVELOPE_H
#define PQCFUZZ_MUTATORS_ENVELOPE_H

#include <cstddef>
#include <cstdint>
#include <string>
#include <vector>

namespace pqcfuzz {

enum class AlgorithmId : uint8_t {
  kUnknown = 0,
  kMlKem512 = 1,
  kMlKem768 = 2,
  kMlKem1024 = 3,
  kMlDsa44 = 4,
  kMlDsa65 = 5,
  kMlDsa87 = 6,
  kSlhDsaSha2_128s = 7,
  kSlhDsaShake_128s = 8,
  kSlhDsaSha2_128f = 9,
  kSlhDsaShake_128f = 10,
  kSlhDsaSha2_192s = 11,
  kSlhDsaShake_192s = 12,
  kSlhDsaSha2_192f = 13,
  kSlhDsaShake_192f = 14,
  kSlhDsaSha2_256s = 15,
  kSlhDsaShake_256s = 16,
  kSlhDsaSha2_256f = 17,
  kSlhDsaShake_256f = 18,
  kAigisEnc1 = 19,
  kAigisEnc2 = 20,
  kAigisEnc3 = 21,
  kAigisEnc4 = 22,
  kAigisSig1 = 23,
  kAigisSig2 = 24,
  kAigisSig3 = 25,
  kNtruHps2048509 = 32,
  kNtruHps2048677 = 33,
  kNtruHps4096821 = 34,
  kNtruHrss701 = 35,
  kFalcon512Compressed = 56,
  kFalcon1024Compressed = 57,
  kFalcon512Padded = 58,
  kFalcon1024Padded = 59,
  kFalcon512Ct = 60,
  kFalcon1024Ct = 61,
  kCrossRsdp1Fast = 64,
  kCrossRsdp1Balanced = 65,
  kCrossRsdp1Small = 66,
  kCrossRsdp3Fast = 67,
  kCrossRsdp3Balanced = 68,
  kCrossRsdp3Small = 69,
  kCrossRsdp5Fast = 70,
  kCrossRsdp5Balanced = 71,
  kCrossRsdp5Small = 72,
  kCrossRsdpg1Fast = 73,
  kCrossRsdpg1Balanced = 74,
  kCrossRsdpg1Small = 75,
  kCrossRsdpg3Fast = 76,
  kCrossRsdpg3Balanced = 77,
  kCrossRsdpg3Small = 78,
  kCrossRsdpg5Fast = 79,
  kCrossRsdpg5Balanced = 80,
  kCrossRsdpg5Small = 81,
};

enum class OracleId : uint8_t {
  kUnknown = 0,
  kMlKemLocalRoundtrip = 1,
  kMlKemCrossExchangeRoundtrip = 2,
  kMlKemTamperedCiphertextImplicitRejection = 3,
  kMlKemBadRandomnessSanity = 4,
  kMlDsaLocalSignVerify = 5,
  kMlDsaCrossVerify = 6,
  kMlDsaMutatedSignatureNegative = 7,
  kMlDsaMutatedMessageNegative = 8,
  kMlDsaMutatedContextNegative = 9,
  kMlDsaOidFieldMutationSanity = 10,
  kMlDsaBadRandomnessSanity = 11,
  kSlhDsaLocalSignVerify = 12,
  kSlhDsaCrossVerify = 13,
  kSlhDsaMutatedSignatureNegative = 14,
  kSlhDsaMutatedMessageNegative = 15,
  kSlhDsaMutatedContextNegative = 16,
  kSlhDsaBadRandomnessSanity = 17,
  kKemDecapsCiphertext = 18,
  kKemDecapsSecretKey = 19,
  kKemEncapsBadRng = 20,
  kKemEncapsZeroPublicKey = 21,
  kKemEncapsPublicKey = 22,
  kKemKeygenBadRng = 23,
  kSigKeygenBadRng = 24,
  kSigSignBadRng = 25,
  kSigSignMessage = 26,
  kSigSignSecretKey = 27,
  kSigVerifyMessage = 28,
  kSigVerifySignature = 29,
  kSigVerifyPublicKey = 30,
  kAigisEncLocalRoundtrip = 31,
  kAigisEncCrossExchangeRoundtrip = 32,
  kAigisEncTamperedCiphertextImplicitRejection = 33,
  kAigisEncBadRandomnessSanity = 34,
  kAigisEncSkNoncanonicalCoefficient = 35,
  kAigisSigLocalSignVerify = 36,
  kAigisSigCrossVerify = 37,
  kAigisSigMutatedSignatureNegative = 38,
  kAigisSigMutatedMessageNegative = 39,
  kAigisSigMutatedContextNegative = 40,
  kAigisSigBadRandomnessSanity = 41,
  kAigisSigExactLength = 42,
  kAigisSigUnusedSignBits = 43,
  kAigisSigCtx256FailureState = 44,
  kAigisSigDeterminismProfile = 45,
  kMlKemImplicitRejectionRelations = 46,
  kAigisEncImplicitRejectionRelations = 47,
  kMlDsaVerifyExactLengths = 48,
  kMlDsaCtxBoundaries = 49,
  kSlhDsaVerifyExactLengths = 50,
  kSlhDsaCtxBoundaries = 51,
  kMlKemRawLengthBoundary = 52,
  kMlKemEkCanonicality = 53,
  kMlDsaHintCanonicality = 54,
  kMlDsaZNormBoundary = 55,
  kMlDsaRndDeterminism = 56,
  kMlDsaPurePrehashSeparation = 57,
  kMlDsaPhOidSeparation = 58,
  kSlhDsaPurePrehashSeparation = 59,
  kSlhDsaPhOidSeparation = 60,
  kMlKemRngFailure = 61,
  kMlDsaRngFailure = 62,
  kSlhDsaRngFailure = 63,
  kNtruKat = 64,
  kNtruLocalRoundtrip = 65,
  kNtruCrossExchange = 66,
  kNtruDpkeMembership = 67,
  kNtruCtPadding = 68,
  kNtruImplicitRejectionExact = 69,
  kNtruPrfKeySeparation = 70,
  kNtruKeyAlgebra = 71,
  kNtruCodecRoundtrip = 72,
  kNtruSkMalformed = 73,
  kNtruLengths = 74,
  kNtruRngAndReplay = 75,
  kNtruFailureState = 76,
  kNtruDpkeFailureOutput = 77,
  kNtruFaultChecks = 78,
  kNtruTimingResources = 79,
  kFalconKat = 100,
  kFalconLocalSignVerify = 101,
  kFalconCrossVerify = 102,
  kFalconMessageSaltBinding = 103,
  kFalconHeaderProfile = 104,
  kFalconPkCoefficients = 105,
  kFalconCompressedCanonicality = 106,
  kFalconFormatLengths = 107,
  kFalconNormEquation = 108,
  kFalconNormBoundaryUnit = 109,
  kFalconHashToPoint = 110,
  kFalconKeyEquation = 111,
  kFalconSkCodec = 112,
  kFalconRngReplay = 113,
  kFalconFailureState = 114,
  kFalconSignedMessageFrame = 115,
  kFalconSamplerArithmetic = 116,
  kFalconFaultChecks = 117,
  kFalconTimingResources = 118,
  kCrossKat = 120,
  kCrossLocalSignVerify = 121,
  kCrossCrossVerify = 122,
  kCrossMessageKeyBinding = 123,
  kCrossExactLengths = 124,
  kCrossPackedFieldRange = 125,
  kCrossVectorPadding = 126,
  kCrossChallengeSampling = 127,
  kCrossCommitmentDigests = 128,
  kCrossDomainTranscript = 129,
  kCrossSeedRebuild = 130,
  kCrossMerkleProof = 131,
  kCrossPathProofConsumption = 132,
  kCrossKeyAlgebra = 133,
  kCrossRngReplay = 134,
  kCrossFailureResources = 135,
  kCrossParallelArithmetic = 136,
  kCrossFaultSeedDisclosure = 137,
  kCrossTiming = 138,
};

struct Envelope {
  uint8_t version = 0;
  AlgorithmId algorithm = AlgorithmId::kUnknown;
  OracleId oracle_id = OracleId::kUnknown;
  uint8_t flags = 0;
  std::vector<uint8_t> seed;
  std::vector<uint8_t> msg;
  std::vector<uint8_t> mutation;
  std::vector<uint8_t> extra;
};

const char *AlgorithmName(AlgorithmId algorithm);
const char *OracleName(OracleId oracle_id);
AlgorithmId AlgorithmIdFromName(const std::string &name);
OracleId OracleIdFromName(const std::string &name);
bool ParseEnvelope(const uint8_t *data, size_t size, Envelope *envelope, std::string *error);

}  // namespace pqcfuzz

#endif
