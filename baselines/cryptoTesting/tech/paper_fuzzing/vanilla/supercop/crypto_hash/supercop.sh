...
fuzz_implementation() {
        ...
        for c_file in $impl/*.c; do
            ...
            $CC $CFLAGS -DCRYPTO_BYTES=$outbytes -c $c_file -o $c_file.o $INCLUDE $LIBS &> $impl/clang.error.log
            ret=$?
            # echo "$ret"
            if [ $ret -ne 0 ]; then
                # cleanup
            ...
            $AFLCC $AFLCFLAGS -DCRYPTO_BYTES=$outbytes -c $c_file -o $c_file.afl.o $INCLUDE $LIBS &> $impl/aflclang.error.log
            ret=$?
            # echo "$ret"
            if [ $ret -ne 0 ]; then
                # cleanup
        ...
        if [ $retry_with_avx -ne 0 ]; then
            ...
            for c_file in $impl/*.c; do
                # need to try compiling with different increasing flags, eg to catch avx etc
                $CC $CFLAGS -DCRYPTO_BYTES=$outbytes -mavx2 -c $c_file -o $c_file.o $INCLUDE $LIBS &>> $impl/clang.error.log
                ret=$?
                # echo "$ret"
                if [ $ret -ne 0 ]; then
                    # cleanup
                ...
                $AFLCC $AFLCFLAGS -DCRYPTO_BYTES=$outbytes -mavx2 -c $c_file -o $c_file.afl.o $INCLUDE $LIBS &>> $impl/aflclang.error.log
                ret=$?
                # echo "$ret"
                if [ $ret -ne 0 ]; then
                    # cleanup
        ...
        $CC $CFLAGS -DCRYPTO_BYTES=$outbytes -o $impl/gen_input.out gen_input.c $(ls $impl/*.S $impl/*.s 2> /dev/null) $(ls $impl/*.o | sed -e 's/[^ ]*.afl.o//g') $INCLUDE $LIBS &>> $impl/clang.error.log
        ret=$?
        # echo "$ret"
        if [ $ret -ne 0 ]; then
            # cleanup
        ...
        $CC $CFLAGS -DCRYPTO_BYTES=$outbytes -o $impl/ParseInput.out bin/ParseInput.a $(ls $impl/*.S $impl/*.s 2> /dev/null) $(ls $impl/*.o | sed -e 's/[^ ]*.afl.o//g') $INCLUDE $LIBS &>> $impl/clang.error.log
        ret=$?
        # echo "$ret"
        if [ $ret -ne 0 ]; then
            # cleanup
        ...
        $AFLCC $AFLCFLAGS -DCRYPTO_BYTES=$outbytes -o $impl/fuzz_harness.afl.out $(ls $impl/*.afl.o) $(ls $impl/*.S $impl/*.s 2> /dev/null) fuzz_harness.c $INCLUDE $LIBS &>> $impl/aflclang.error.log
        ret=$?
        # echo "$ret"
        if [ $ret -ne 0 ]; then
            # cleanup
        ...
        do
            if [ ! -f $impl/gen_input.out ]
            then
                echo "$impl/gen_input.out doesn't exist"
            fi
            # $impl/GenInput.out $impl/fuzzinputs/test.bin $bytes $outbytes &>> $impl/supercop.geninput.log
            if ! ( false || $impl/gen_input.out $impl/fuzzinputs/test.bin $bytes &>> $impl/supercop.geninput.log ) >/dev/null 2>&1; then
            ...
            errorline=$($AFLFUZZ -i $impl/fuzzinputs/ -o $impl/fuzzoutputs/ -D -- $impl/fuzz_harness.afl.out @@);
            # echo "$errorline";
